...
#' @param support A string denoting the column name for the support. Defaults to "support".
...
manhattan_rmip <- function(x, chr="CHR", bp="BP", p="P", snp="SNP", support="SUPPORT", 
  ...
  if (!is.null(x[[snp]])) d = data.frame(CHR=x[[chr]], BP=x[[bp]], P=x[[p]], pos = NA, index = NA ,SNP=x[[snp]], stringsAsFactors = FALSE) else 
    d = data.frame(CHR=x[[chr]], BP=x[[bp]], P=x[[p]], SUP=x[[support]], pos = NA, index = NA)
  ...
  d$Col <- rbPal(10)[as.numeric(cut(d$SUP,breaks = 10))] 

    # Add points to the plot
  if (nchr==1) {
    with(d, points(pos, SUPPORT, pch=20, col=col[1], ...))
  } else {
    # if multiple chromosomes, need to alternate colors and increase the color index (icol) each chr.
    icol=1
    for (i in unique(d$index)) {
      #with(d[d$index==unique(d$index)[i], ], points(pos, logp, col=col[icol], pch=20, ...))
      points(d[d$index==i,"pos"], d[d$index==i,SUP], col=d[d$index==i,Col], pch=20, ...)
      icol=icol+1
    }
  }
